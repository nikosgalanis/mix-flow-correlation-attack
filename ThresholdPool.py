#This version outputs randomly chosen m packets if n=m+f. But runs forever for some reason even though last output is Batch 1000. 
...
def threshold_pool(n: int, m: int, f: int):
    ...
    with open('input.txt', 'r') as input_file, open('output.txt', 'w+') as output_file:
        ...
        while count < n:  # break if count >= n
            ...
            for i, (src, dest, time_str, msg) in enumerate(lines):
                ...
                if (time - max_time).total_seconds() <= m and count < n:
                    ...
                    if len(pool) == n:
                        batch += 1
                        # randomly select m packets from the pool
                        selected_msgs = random.sample(pool, m)

                        # write the messages along with the ending time
                        output_file.write(f'BATCH {batch} - ThresholdPool\n')
                        for src, dest, time_str, msg in selected_msgs:
                ...
                # check if batch number is equal to n/m and break out of the loop
                if batch == n // m:
                    break
...
def mix(n_messages, n_nodes, random_time, thres, f):
    ...
    print(f"All {n_messages} messages processed.")
